...
class ZPLFunctions:
    def BuildLabel(self, listvar, qty):
        OrderType, DeliveryNumber, BagQty, BoxQty, DryQty, CustName, CustStreet, CustZip, CustCity, CustPhone, Operator, Date, OrderNr = listvar
        ...
        self.zpl.add_zpl_raw('^MMT^PW831^LL0440^LS0'
                             ...
                             '^FO496,377^GB329,0,5^FS^FO6,233^GB818,0,5^FS^FO496,305^GB329,0,5^FS^FO165,236^GB0,216,5^FS^FO491,236^GB0,216,5^FS')

        #Order quantity data fields
        self.zpl.add_field_origin(17, 290)
        self.zpl.add_field_block(180,1,0,'C')
        self.zpl.add_zpl_raw('^A0N,114,140')
        self.zpl.add_field_data(BagQty) 

        self.zpl.add_field_origin(180, 290)
        self.zpl.add_field_block(180,1,0,'C')
        self.zpl.add_zpl_raw('^A0N,114,140')
        self.zpl.add_field_data(BoxQty) 

        self.zpl.add_field_origin(342, 290)
        self.zpl.add_field_block(180,1,0,'C')
        self.zpl.add_zpl_raw('^A0N,114,140')
        ...
        self.zpl.add_field_origin(567, 388)
        self.zpl.add_field_block(200,1,0,'C')
        self.zpl.add_zpl_raw('^A0N,30,40')
        ...
        if OrderType == 'D':        
            ...
            self.zpl.add_zpl_raw('^FO328,27^GB0,427,5^FS'                                 
            ...
            self.zpl.add_zpl_raw('^FT347,72^A0N,35,35^FH\^FD' + str(CustName) + '^FS'
                                 ...
                                 '^FT465,155^A0N,35,35^FH\^FD' + str(CustCity) + '^FS'
                                 '^FT374,215^A0N,35,35^FH\^FD' + str(CustPhone) + '^FS')
        elif OrderType == 'C':
            #Row separators, labeltext
            self.zpl.add_zpl_raw('^FO328,236^GB0,218,5^FS'
        ...
        self.zpl.add_zpl_raw('^FT25,441^A0N,24,36^FB125,1,0,C^FH^FDTASSEN^FS'
                             '^FT198,441^A0N,24,36^FB101,1,0,C^FH^FDDOZEN^FS'
                             '^FT358,441^A0N,24,36^FB108,1,0,C^FH^FDDROOG^FS'
                             '^FT536,295^A0N,24,36^FB247,1,0,C^FH^FDINGEPAKT DOOR^FS'
                             '^FT537,367^A0N,24,36^FB245,1,0,C^FH^FDORDERNUMMER^FS'
                             '^LRY^FO16,417^GB144,0,29^FS^LRN'
                             '^LRY^FO338,417^GB148,0,29^FS^LRN'
                             '^LRY^FO175,417^GB148,0,29^FS^LRN'
